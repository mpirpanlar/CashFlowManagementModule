...
{
    internal static class CollectionOrderUdFields
    {
        public const string PaymentDate = "UD_PaymentDate";
    }

    public static class BankReceiptCollectionOrderHelper
    {
        public const short ReceiptType = 20;
        ...
        public static bool IsCollectionOrderReceipt(BusinessObjectBase businessObject)
        {
            if (businessObject?.CurrentRow?.Row == null) return false;
            return IsCollectionOrderReceipt(businessObject.CurrentRow.Row);
        }

        public static bool IsCollectionOrderReceipt(DataRow headerRow)
        {
        ...
        public static bool IsCollectionOrderReceiptType(short receiptType)
        {
            return receiptType == ReceiptType;
        }

        public static bool IsCollectionOrderContext(PMBase pm)
        {
            ...
            if (pm.ActiveBO is BusinessObjectBase businessObject && IsCollectionOrderReceipt(businessObject))
                return true;
        ...
        public static bool ShouldLockCollectionOrder(IBusinessObject businessObject)
        {
        ...
        {
            if (!IsCollectionOrderReceipt(businessObject)) return;
        ...
        public static void EnsureCollectionOrderValueFillerSetup(BusinessObjectBase businessObject)
        {
            if (!IsCollectionOrderReceipt(businessObject)) return;
        ...
        {
            if (!HasUdColumn(itemRow, CollectionOrderUdFields.PaymentDate) || !IsUsableDataRow(itemRow)) return;
            if (!itemRow.IsNull(CollectionOrderUdFields.PaymentDate)) return;

            itemRow[CollectionOrderUdFields.PaymentDate] = new DateHelper().GetToday();
        }
        ...
        public static void RestoreTodayPaymentDateIfSyncedFromReceiptDate(DataRow itemRow)
        {
            ...
            DateTime paymentDate = Convert.ToDateTime(itemRow[CollectionOrderUdFields.PaymentDate]).Date;
        ...
        public static Dictionary<DataRow, object> SnapshotItemPaymentDates(BusinessObjectBase businessObject)
        {
            ...
            if (!HasUdColumn(itemTable, CollectionOrderUdFields.PaymentDate))
                return snapshot;
        ...
        public static void RestoreItemPaymentDates(Dictionary<DataRow, object> snapshot)
        {
            ...
            {
                if (!IsUsableDataRow(entry.Key) || !HasUdColumn(entry.Key, CollectionOrderUdFields.PaymentDate))
                    continue;

                entry.Key[CollectionOrderUdFields.PaymentDate] = entry.Value ?? DBNull.Value;
            }
        }

        public static void ProtectItemPaymentDateOnReceiptDateChange(DataRow itemRow, Dictionary<DataRow, object> snapshot)
        {
            if (!HasUdColumn(itemRow, CollectionOrderUdFields.PaymentDate) || !IsUsableDataRow(itemRow))
                return;
            ...
            {
                itemRow[CollectionOrderUdFields.PaymentDate] = savedValue ?? DBNull.Value;
                return;
            }

            if (itemRow.HasVersion(DataRowVersion.Original))
            {
                object originalValue = itemRow[CollectionOrderUdFields.PaymentDate, DataRowVersion.Original];
                itemRow[CollectionOrderUdFields.PaymentDate] = originalValue == null || originalValue == DBNull.Value
                    ? DBNull.Value
                    : originalValue;
            }
        }
