...
{
    internal static class BankReceiptCollectionOrderApprovalHelper
    {
        public static bool CanToggleHeaderApproval(BankReceiptPM pm)
        {
            return BankReceiptCollectionOrderHelper.CanToggleHeaderApproval(pm?.ActiveBO?.CurrentRow?.Row);
        }
        ...
        public static void ExecuteHeaderApprovalToggle(BankReceiptPM pm, ISysCommandParam obj)
        {
            ...
            if (!BankReceiptCollectionOrderHelper.CanToggleHeaderApproval(headerRow))
                throw new LiveCommandItemException(CollectionOrderTerminology.HeaderApprovalDeniedMessage);
            ...
            if (newApproved == 0
                && BankReceiptCollectionOrderHelper.GetPersistedApprovedValue(headerRow) == 1
                && !BankReceiptCollectionOrderHelper.HasHeaderApprovedEditRight())
                throw new LiveCommandItemException(CollectionOrderTerminology.HeaderApprovalDeniedMessage);

            BankReceiptCollectionOrderHelper.ApplyHeaderApprovalChange(headerRow, newApproved);

            if (pm.ActiveBO.PostData() != PostResult.Succeed)
                throw new LiveCommandItemException(pm.ActiveBO.ErrorMessage);

            pm.ActiveBO.Get(Convert.ToInt64(headerRow["RecId"]));
            RefreshCollectionOrderApprovalUi(pm);
        }

        public static void RefreshCollectionOrderApprovalUi(BankReceiptPM pm)
        {
            if (pm == null) return;
            ...
            if (pm is PMDesktop pmDesktop)
                pmDesktop.SetViewEnabled(!BankReceiptCollectionOrderHelper.ShouldLockCollectionOrder(pm.ActiveBO));

            if (pm.contextMenu == null) return;

            DataRow headerRow = pm.ActiveBO?.CurrentRow?.Row;
            bool canToggleHeaderApproval = BankReceiptCollectionOrderHelper.CanToggleHeaderApproval(headerRow);
        ...
        static byte ResolveNewApprovedValue(DataRow headerRow, ISysCommandParam obj)
        {
            ...
            byte currentApproved = BankReceiptCollectionOrderHelper.GetApprovedValue(headerRow);
